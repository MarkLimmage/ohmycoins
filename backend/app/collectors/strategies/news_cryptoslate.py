        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                for link in soup.find_all('a', href=True):
                    href = link['href']
                    if href.startswith('https://cryptoslate.com/'):
                        # Exclude non-news
                        if not any(x in href for x in ['/coins/', '/people/', '/companies/', '/category/', '/tag/', '/feed/']):
                ...
                for article_url in links:
                    try:
                        ...
                        content_div = detail_soup.find('div', class_='post3-content') or detail_soup.find('div', class_='post-body')
                        if content_div:
                        ...
                        published = None
                        meta_date = detail_soup.find('meta', property='article:published_time')
                        if meta_date:
                        ...
                        logging.error(f"Error scraping CryptoSlate article {article_url}: {e}")
                        continue
