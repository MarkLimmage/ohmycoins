        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                for link in soup.find_all("a", href=True):
                    href = link["href"]
                    if isinstance(href, str) and "/news/" in href:
                        full_url = urljoin("https://finance.yahoo.com", href).split(
                            "?"
                        )[0]
                ...
                for article_url in links:
                    try:
                        ...
                        article_container = (
                            detail_soup.find(
                                "div", class_=lambda x: x and "caas-body" in x.lower()
                            )
                            or detail_soup.find(
                                "div",
                                class_=lambda x: x and "article-body" in x.lower(),
                            )
                        ...
                        logging.error(
                            f"Error scraping Yahoo article {article_url}: {e}"
                        )
                        continue
