        ...
        with Session(engine) as session:
            ...
            for db_coll in db_collectors:
                try:
                    ...
                    if not strategy_cls:
                        logger.warning(
                            f"Plugin {db_coll.plugin_name} not found for collector {db_coll.name}"
                        )
                        continue
                    ...
                    if "exchange" in db_coll.plugin_name.lower():
                        ...
                        "news" in db_coll.plugin_name.lower()
                        or "reddit" in db_coll.plugin_name.lower()
                    ):
                        ledger = "human"

                    adapter = StrategyAdapterCollector(
                        strategy, ledger_name=ledger, default_config=db_coll.config
                    )

                    # Override name to match DB name so distinct instances work
                    adapter.name = db_coll.name

                    # Register/Update Job using CronTrigger
                    try:
