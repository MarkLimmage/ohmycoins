...
class LangGraphWorkflow:
    ...
    async def _dispatch_alerts_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if result.success:
                ...
                state["messages"].append(
                    {"role": "system", "content": f"Alert dispatched: {result.message}"}
                )
            else:
            ...
            state["messages"].append(
                {"role": "system", "content": f"Error dispatching alert: {str(e)}"}
            )
    ...
    async def stream_execute(self, initial_state: AgentState | None = None, session_id: str | None = None):
        ...
        """
        from langgraph.types import Command  # noqa: F401 — kept for future use
