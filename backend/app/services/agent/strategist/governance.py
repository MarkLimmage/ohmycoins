...
class GovernanceEvaluator:
    ...
    def evaluate(self, result: BacktestResult) -> GovernanceDecision:
        ...
        if abs(result.max_drawdown) > self.config.max_drawdown_percent:
            reasons.append(f"Max Drawdown {abs(result.max_drawdown):.2%} > Limit {self.config.max_drawdown_percent:.0%}")

        # 3. Win Rate Check
        if result.win_rate < self.config.min_win_rate:
            reasons.append(f"Win Rate {result.win_rate:.2%} < Minimum {self.config.min_win_rate:.0%}")
