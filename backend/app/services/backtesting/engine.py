...
class BacktestService:
    ...
    def _fetch_data(
        self, coin_type: str, start_date: datetime, end_date: datetime
    ) -> pd.DataFrame:
        """
        ...
        data = [
            {
                ...
                "open": float(
                    r.last
                ),  # Proxying open with last/close if open not available in current model
    ...
    def run_backtest(
        ...
        strategy_fn: Callable[[pd.DataFrame, dict], pd.DataFrame] = None,
        ...
        """
        logger.info(
            f"Starting backtest for {config.strategy_name} on {config.coin_type}"
        )
        ...
        result = self._calculate_performance(
            df, config.initial_capital, fee_rate, slippage
        )
    ...
    def _calculate_performance(
        ...
        return BacktestResult(
            ...
            equity_curve=equity_curve[
                -100:
            ],  # Just return last 100 for brevity in result, or full?
    ...
    def _extract_trades(
        self, df: pd.DataFrame, fee_rate: float, slippage: float
    ) -> list[dict[str, Any]]:
        """
