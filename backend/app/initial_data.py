...
def seed_collectors(session: Session) -> None:
    ...
    collectors_to_seed = [
        ...
        {
            "name": "CryptoPanic",
            "description": "Community-voted crypto news sentiment from CryptoPanic",
            "plugin_name": "human_cryptopanic",
            "is_enabled": True,
            "schedule_cron": "*/30 * * * *",
            "config": {"currencies": "BTC,ETH,SOL,AVAX,LINK,AAVE,UNI,DOT,ADA,MATIC"},
            "status": "idle",
        },
        {
            "name": "SnapshotGovernance",
            "description": "DeFi governance proposals from Snapshot",
            "plugin_name": "governance_snapshot",
            "is_enabled": True,
            "schedule_cron": "0 */6 * * *",
            "config": {
                "spaces": [
