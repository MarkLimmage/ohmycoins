...
def promote_algorithm(
    ...
    try:
        ...
        if not run:
            raise HTTPException(
                status_code=404,
                detail=f"MLflow run_id {request.mlflow_run_id} not found in local registry.",
            )
        ...
        raise HTTPException(
            status_code=503, detail="MLflow Tracking Server unavailable."
        )
        ...
        if (
            ...
            or "max retries exceeded" in str(e).lower()
        ):
            raise HTTPException(
                status_code=503, detail="MLflow Tracking Server unavailable."
            )
        raise HTTPException(
            status_code=404,
            detail=f"MLflow run_id {request.mlflow_run_id} not found or error accessing registry. {str(e)}",
        )
    ...
    try:
        ...
        client.transition_model_version_stage(
            name=request.algorithm_name, version=model_version.version, stage="Staging"
        )
    except Exception as e:
