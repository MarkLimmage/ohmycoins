...
def create_llm_credentials(
    *,
    ...
    logger.info(
        f"LLM credential created for user {current_user.id}, provider={credentials_in.provider}, credential_id={db_credentials.id}"
    )
...
def list_llm_credentials(
    *,
    ...
    credentials_list = session.exec(
        select(UserLLMCredentials).where(
            UserLLMCredentials.user_id == current_user.id, UserLLMCredentials.is_active
        )
    ).all()
...
def delete_llm_credential(
    *,
    ...
    logger.info(
        f"LLM credential deleted for user {current_user.id}, provider={credential.provider}, credential_id={credential_id}"
    )
