...
def request_promotion(
    *,
    ...
    if existing_promotion:
        raise HTTPException(
            status_code=400,
            detail="A pending promotion request already exists for this algorithm",
        )

    promotion = StrategyPromotion.model_validate(
        promotion_in, update={"created_by": current_user.id}
    )
...
def list_promotions(
    *,
    ...
    if current_user.is_superuser:
        ...
        statement = select(StrategyPromotion).where(
            StrategyPromotion.created_by == current_user.id
        )
...
def review_promotion(
    *,
    ...
    if not current_user.is_superuser:
        raise HTTPException(
            status_code=403, detail="Only the Architect can review promotions"
        )
