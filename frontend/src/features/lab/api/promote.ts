...
export const promoteRunToFloor = async (payload: PromoteRunPayload): Promise<PromoteRunResponse> => {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.detail || `Failed to promote run: ${response.statusText}`);
  }
