...
import type { LabCell, LabStage } from "../types"
...
export const LabStageRow: React.FC<LabStageRowProps> = ({
  cells,
  isActive,
  label,
}) => {
  ...
  return (
    <Box
      ...
      borderLeftWidth="4px"
      borderColor={
        isActive ? "blue.500" : cells.length > 0 ? "green.500" : "gray.200"
      }
    ...
    >
      <VStack align="start" gap={4} w="100%">
        <HStack
          gap={2}
          pb={2}
          borderBottomWidth={cells.length > 0 ? "1px" : "0px"}
          w="100%"
        >
          {isActive ? (
            <Icon
            ...
            <Icon
              as={cells.length > 0 ? FiCheckCircle : FiCircle}
              color={cells.length > 0 ? "green.500" : "gray.400"}
            />
          )}
          <Heading
            size="sm"
