...
- ⚡ **Real-Time Communication**: WebSocket patterns and fallback strategies

### What This Document Does NOT Contain:
- ❌ Exact request/response schemas (use Pydantic models in OpenAPI)
- ❌ Complete endpoint catalog (auto-generated by FastAPI)
...
| Token expired | Automatic silent refresh | 401 → POST /api/v1/auth/refresh |
...
- Button with inline spinner: `[⏳ Saving...]`
...
```
[Input Field]
❌ API keys must start with 'sk-' or 'sk-proj-'
```

**Banner Errors** (Page-level issues):
```
⚠️ Unable to load Ledger data
...
```
🔴 Background sync failed. Will retry in 30s.
```
...
- Link: "How to get your API key" → https://platform.openai.com/api-keys
...
- Modal shows progress:
  ...
  ⏳ Subscribing to Exchange Ledger...
...
- Show inline errors:
  ...
  ❌ Must be at least 50% of max position size ($1500)
